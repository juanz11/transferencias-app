...
class TransferenciaPedidoController extends Controller
{
    ...
    public function createVisitador()
    {
        ...
        $usados = Transferencia::whereNotNull('transferencia_numero')
            ->whereBetween('transferencia_numero', [8501, 20000])
            ->pluck('transferencia_numero')
            ->map(fn($n) => (int) $n)
            ->toArray();

        $numerosDisponibles = [];
        for ($i = 8501; $i <= 20000; $i++) {
    ...
    public function storeVisitador(Request $request)
    {
        ...
        $request->validate([
            ...
            'fecha_correo' => 'required|date',
            'fecha_transferencia' => 'required|date',
            'transferencia_numero' => 'required|integer|min:8501|max:20000|unique:transferencias,transferencia_numero',
            'productos' => 'required|array',
            'productos.*.id' => 'required|exists:productos,id',
            'productos.*.cantidad' => 'required|numeric|min:1',
            'productos.*.descuento' => 'nullable|numeric|min:0'
